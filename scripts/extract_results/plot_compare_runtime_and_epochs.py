...
grouped = df[df.features == "+ESF"].reset_index(drop=True)
...
# plt.show()
fig.savefig(os.path.join("../../results/Compare_model_runtime_v%s.pdf" % prefix), format="pdf",
            bbox_inches="tight")
plt.close(fig)
...
fig.savefig(os.path.join("../../results/Compare_epochs_v%s.pdf" % prefix), format="pdf",
            bbox_inches="tight")
plt.close(fig)
