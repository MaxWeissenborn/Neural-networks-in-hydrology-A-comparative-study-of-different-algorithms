...
for value in itertools.product(modelNames, bs_list):
    ...
    for i in range(2):
        ...
        #     sub_plot.yaxis.tick_right()
            ...
            if ext == "+ESF":
                # sub_plot.set_xlim(xmin=0, xmax=16) # when plotting all models
                sub_plot.set_xlim(xmin=0, xmax=14) # this forThesis only
                ...
                sub_plot.legend(handles=patches, loc="center", frameon=False, bbox_to_anchor=(0.65, 0.249))
                rect = matplotlib.patches.FancyBboxPatch((5, 17), width=8.2, height=10, linewidth=.4,
            ...
            for k in j:
                ...
                if width != 0.0:
                    if abs(width) <= 0.1:
                        ...
                        if i == 0:
                            axes[i].text(width-0.1, k.get_y()+k.get_height()-0.2, width.round(1), ha='right', size=14, weight="bold", color="black",)
                        else:
                            axes[i].text(width + 0.1, k.get_y() + k.get_height() - 0.2, width.round(1), ha='left', size=14,
                                         weight="bold", color="black", )
