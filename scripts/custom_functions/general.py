...
def write_db_train(current_run_settings, par, db_progress_dict, train_norm, val_norm, run=None):
    ...
    # mod_args["T"] = 100 # todo muss wieder raus
...
def process_train_data(model_settings, mod_import, skip_db_train, model_name, train_norm, val_norm):
    ...
    if not skip_db_train:
        ...
        for f in sorted_nicely([os.path.basename(x) for x in glob.glob(str(path_join([train_db_path, "*.hdf5/"])))]):
            db_progress_dict[model_name].append(f)
...
def process_testing_data(model_settings, skip_db_test, model_name, test_norm, train_db_path):
    ...
    if not skip_db_test:
        ...
        for f in sorted_nicely(
                [os.path.basename(x) for x in glob.glob(str(path_join([model_settings.test_db_path,
                                                                       "*.hdf5/"])))]):
            db_progress_dict[model_name].append(f)

        par_path_list = sorted_nicely(
            [f for f in glob.glob(str(path_join([train_db_path, "parameter*.yml/"])))])
...
def train_model(mod_import, model_settings, progress_df, db, model_name, bs, epochs):
    ...
    def lr_scheduler(epoch, lr, warmup_epochs=2, decay_epochs=11, initial_lr=1e-6, base_lr=5e-4, min_lr=5e-5):
        ...
        # if epoch <= warmup_epochs + decay_epochs: this line should be choosen to make sure that all decay epochs are used.
    ...
    if not is_trained:
        ...
        if go_ahead:
            ...
            pickle.dump([model_name, db_name, bs, epochs, np.nan, np.nan],
...
def validate_model(db_list, args, progress_df, epochs, bs, run):
    ...
    if os.path.isdir(path_join([train_path, m_name])):
        is_validated = check_progress_df(progress_df, m_name, db_name, bs, epochs, validating=True)

        if not is_validated:

            # with tf.device('/device:GPU:0'):
            model = load_model(path_join([train_path, m_name]), compile=False)
            model.compile(loss=CustomLoss(), optimizer=tf.keras.optimizers.Adam(learning_rate=args["lr"]))

            with h5py.File(db, 'r') as db:
                x = db["X_val"][...]
                y = db["Y_val"][...].flatten()
            ...
            output_path = path_join([train_path, "validation"])
            make_dir(output_path)

            pickle.dump(y_unscaled, open(path_join([output_path, "validation_observation_unscaled.pkl"]), "wb"))
            pickle.dump(p_unscaled, open(path_join([output_path, "validation_prediction_unscaled.pkl"]), "wb"))
            ...
            for i in range(n):
                ...
                if len(y_sample) * n != len(y):
                    logger.error(
                        "--> Validation samples have no equal sizes, check 'train_samples' within global Settings")
                    break
                ...
                river_ids = get_river_ids(args["path_to_training_df"])
                line_plotting(target_df, i + 1, output_path, kge, river_ids)

                # save results
                pickle.dump(kge_list, open(path_join([output_path, "KGE_validation-results.pkl"]), "wb"))
            ...
            out = [m_name, db_name, bs, epochs, True, np.nan]
            if is_tested:
                out = [m_name, db_name, bs, epochs, True, True]

            pickle.dump(out, open(path_join(["data", "tmp", f_name + ".pkl"]), "wb"))

        else:
            logger.info('--> Validation was already processed and will be skipped')
    ...
    if os.path.isdir(path_join([train_path, m_name])):
        ...
        if not is_tested:
            ...
            for i in range(n):
                ...
                # save results
                pickle.dump(kge_list, open(path_join([output_path, "KGE_test-results.pkl"]), "wb"))
            ...
            f_name = m_name + "-" + db_name + "-" + str(bs) + "-" + str(epochs)
            is_validated = check_progress_df(progress_df, m_name, db_name, bs, epochs, validating=True)
            out = [m_name, db_name, bs, epochs, np.nan, True]
            if is_validated:
                out = [m_name, db_name, bs, epochs, True, True]
...
def violin_plotting(path, data):
    ...
    # ax = sns.violinplot(x="KGE", palette="Set3", data=data,
...
def check_progress_df(df, model_name, db, bs, epochs, validating=False, testing=False):
    a = df[df['modelName'] == model_name]
    b = a[a['dbName'] == db]
    c = b[b['batchSize'] == bs]
    d = c[c['epochs'] == epochs]
    e = d[d['validation'] == True]
    f = d[d['testing'] == True]
    ...
    if len(files) > 0:
        for file in [f for f in files]:
            ...
            if _[4] == True and len(df_tmp) > 0:
                idx = df_tmp.index[0]
                df.loc[idx, "validation"] = True
            if _[5] == True and len(df_tmp) > 0:
                idx = df_tmp.index[0]
