...
def save(data, filename):
    ...
    data.to_csv(Path.joinpath(output_path, 'delete_NO NA - VALIDATION NOT FOR TRAIN - ' + name), encoding='utf-8')
...
for f in filenames:
    temp_dict = load_data(f)
    name_list = name_list + check_for_nan(temp_dict)
    data_dict[f] = temp_dict
...
df_catchment = pd.read_csv(Path.joinpath(output_path, "NO NA cleaned_catchment_attributes_num.csv"),  sep=",")
...
if add_static_features:
    ...
    for k, v in df_dict.items():
        temp_df = pd.DataFrame(np.repeat(df_catchment.loc[k].values[np.newaxis, ...], v.shape[0], axis=0), columns=new_cols)
...
out_name = "%s - NO NA - VALIDATION NOT FOR TRAIN - Complete River Data - %s - %s.pkl" % (prefix, start_y, end_y)
...
out_name = "%s - NO NA - VALIDATION NOT FOR TRAIN - Complete River Data as Dataframe - %s - %s.pkl" % (prefix, start_y, end_y)
