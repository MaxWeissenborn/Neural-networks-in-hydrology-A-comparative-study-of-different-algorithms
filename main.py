...
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # '-1' for GPU
...
def main():
    ...
    # number of features
    d = train.shape[1] - 2  # - gauge_id and target todo: this is hardcoded
    globSettings.d = d
    ...
    if train_prog_file.is_file():
        ...
        progress_df = pd.DataFrame(columns=['modelName', 'dbName', 'batchSize', 'epochs', 'validation', 'testing'])
    ...
    if model_names:
        ...
        for modelName in model_names:
            ...
            # get all databases for imported model
            db_list = sorted_nicely([db for db in glob.glob(str(path_join([train_db_path, "*.hdf5/"])))])
            ...
            if not skip_validation:
                ...
                if model_settings.validation:
                    logger.info('Processing validation for model: %s' % modelName)

                    with tqdm(total=model_settings.runs * len(bs_list) * len(epochs_list), file=sys.stdout) as pbar:
                        for db, bs, epochs in itertools.product(db_list, bs_list, epochs_list):
                            val_args = [db, model_settings, progress_df, epochs, bs]
                            validate_model(*val_args)
                            pbar.update(1)

                    progress_df = update_progress(tmp_dir, progress_df, train_prog_file)

                else:
                    logger.info('Validation was globally deactivated, therefore it will be skipped')

            else:
                logger.info('--> Model validation is in debug mode and will be skipped')
...
if __name__ == '__main__':
    ...
    # debug variables
    skip_db_train = False  # includes validation db
    skip_db_test = False
    skip_training = False
